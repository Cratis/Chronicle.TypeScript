...
For model-bound `@childrenFrom`, an automatically mapped child whose identifier is a discoverable `id`/`Id`, or supplied as `identifiedBy`, gets its identifier from `$eventContext(EventSourceId)` when the child key defaults to the event source id. An explicit event key instead maps to that event property unless it already matches the identifier name; if no `id` is found, a child property matching the event key becomes `IdentifiedBy`. TypeScript has no `[Key]` decorator: pass `identifiedBy` for a differently named identifier, and use an initialized property or `@field` so convention-based `id`/`Id` is discoverable at runtime. Explicit identifier mappings and `@noAutoMap` on the child or its declaring class take precedence. This adds a property mapping to the registered child definition for affected projections; the kernel compares changed definitions and re-evaluates them.
