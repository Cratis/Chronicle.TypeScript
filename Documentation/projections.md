...
- Event-context mappings with `@setFromContext(Event, 'occurred')`, `.set(...).toEventContextProperty('occurred')`, and context-derived keys with `.usingKeyFromContext('eventSourceId')` or `.usingParentKeyFromContext('eventSourceId')`. These names use the TypeScript `EventContext` spelling; the client sends `$eventContext(Occurred)` and `$eventContext(EventSourceId)` to the kernel. Nested context paths such as `causedBy.subject` are supported, including recursive `causedBy.onBehalfOf` identity members. Registration checks the path syntax and root against the TypeScript `EventContext`, identity members (`subject`, `name`, `userName`, `onBehalfOf`). `causation` is a collection in the kernel, so it can be mapped only as a whole; paths into it, such as `causation.type`, are rejected. Only the kernel's `Week()` derived function is accepted (case-insensitively, emitted as `Week()`). Unknown names in those checked positions and invalid function names fail during projection registration with the projection/read model and invalid path in the error; other nested CLR paths are not checked against their member types. The error's cause is `InvalidEventContextPropertyError`, exported from `@cratis/chronicle` and `@cratis/chronicle/projections`. Earlier client versions sent expressions the kernel did not recognize, or treated model-bound context names as event-content properties.
