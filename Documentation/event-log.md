...
- `appendResult.waitForCompletion(timeoutMs?)` waits for every observer affected by an append to catch up or fail before you read a read model back. It resolves immediately for an unsuccessful append and rejects when the timeout (5 seconds by default) passes. See [Failed Partitions](./failed-partitions.md).
