...
Against kernel 18.4.1 an unnarrowed tail read reports the tail of the whole sequence; a legacy-scoped read reports only that stream, and reports `EventSequenceNumber.unset` when that stream holds no events. `getNextSequenceNumber()` already maps `unset` to `EventSequenceNumber.first`; compare against `EventSequenceNumber.unset` yourself when you read a scoped tail directly. The event source type argument `getForEventSourceIdAndEventTypes()` accepts narrows the read from kernel 18.5.0 onward. Before that the query carried no such field on the wire, so the argument was accepted and silently discarded — a caller that narrowed by it got every source type back with no way to tell — and against an older kernel it still does nothing. `getFromSequenceNumber()` and `hasEventsFor()` carry no route dimensions at all and are unchanged.
