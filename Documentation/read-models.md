...
Queries fill every property the model declares from the stored read model. As of `@cratis/chronicle` v6.14.1, standard-decorated models whose mappings are only on properties register before construction if their classes are exported by discovered modules. With compiled JavaScript and no discovery patterns, explicitly register the class before creating the event store; importing it alone is not enough. Legacy decorators still register property-only models on import. For unresolved standard mappings, store creation emits one diagnostic warning with the mapped properties and event types; see [Artifact discovery](./getting-started.md#artifact-discovery) for registration options and how to enable diagnostic logging.
