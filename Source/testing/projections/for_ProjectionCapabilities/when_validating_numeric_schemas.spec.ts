...
describe('when validating the numeric capability matrix', () => {
    ...
    it('should admit mappings into unformatted Number fields generated by TypeScript', () => {
        const { compiled, definition } = compileDeclarative(builder => builder.from(Changed, from => from
            .set(model => model.total).to(event => event.name)
            .set(model => model.quantity).to(event => event.quantity)));
        const schema = JSON.parse(compiled.readModels[0].Schema) as { properties: { total: { type: string; format?: string }; quantity: { type: string; format?: string } } };
        ('format' in schema.properties.total).should.be.false;
        ('format' in schema.properties.quantity).should.be.false;
        (() => ProjectionCapabilities.validate(compiled, definition)).should.not.throw();
        // The oracle's compatible-mappings fixture also tests formatted int32/uint32 sources into unformatted Number targets.
        compiled.eventSchemas.get(definition)!.get('capability-changed:1:0')!.schema.properties!.quantity = { type: 'integer', format: 'int32' };
        (() => ProjectionCapabilities.validate(compiled, definition)).should.not.throw();
    });
